...
public class AmountController {
	...
	public void updateAmount(int userId,String id,String pass) {
		int fee = (int)mc.selectFee(id, pass);
		Amount a = new Amount();
		a.setUserId(userId);
		a.setPayFee(fee);
		a.setRestFee(fee);
		int re = as.updateAmount(a);
		if(re>0) {
			new MainMenu().successDisplay("변경된 정보를 토대로 보험료 변경이");
		}else {
			new MainMenu().failureDisplay("변경된 정보를 토대로 보험료 변경이");
