...
public class MainMenu{
	...
	public void insertCar() {
		...
		System.out.println("차 종류(1.승용차, 2.승합차, 3.화물차, 4.특수차)");
	...
	public void insertDriveWeight() {
		...
		dw.insertDriveWeight(companyId, car, min, max, score);
	}
	public void insertAccidentWeight() {
		...
		aw.insertAccidentWeight(companyId, car, min, max, score);
	}
	...
	public void updateJoinMember(MemberVo m) {
		...
		sc.nextLine();
		int result = mc.updateJoinMember(m.getUserId(),id,pass, address, phone, career, car, score);
		if(result>0) {
			ac.updateAmount(m.getUserId(),id,pass);
		}
		}catch(InputMismatchException e) {
		    System.out.println("숫자를 정확히 입력해주세요.");
		    sc.nextLine();
	...
	public boolean payFee(MemberVo m) {
		...
		if(ac.selectCount(m)==5) {
			mc.deleteJoinMemeber(m.getId(),m.getPass());
			System.out.println("납부 횟수 초과로 자동 탈퇴 처리 되었습니다.");
			return false;
		}
		return true;
