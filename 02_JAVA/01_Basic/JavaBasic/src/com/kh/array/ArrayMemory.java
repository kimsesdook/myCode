...
public class ArrayMemory {
	...
	public static void main(String[] args) {
		 ...
		 * 다른 메모리의 주소값을 담고 있는 변수를 참조변수라고 함.
		 * 
		 * 기본 자료형(원시타입) : int , double ,float....
		 * -> 실제 리터럴값을 바로 담을 수 있음.
		 * 
		 * 그외 자료형(String, Scanner, int[].....)-> 참조타입
		...
		//배열 생성시 따로 초기화하지 않아도 기본값이 담겨져 있음
		//Heap이라는 메모리 공간은 절대 빈공간을 허용하지 않기 때문에
	    ...
	    /*
	     * 연결이 끊어진 기존 배열의 메모리 공간은 heap영역에 남아있다.(어떤 변수에도 참조되지 않은 상태)
	     * => 일정 시간이 지나면 "GC(가비지컬렉터)"가 알아서 회수한다.
	     * => 자바에서의 "자동 메모리관리"  특징=> 개발자는 코드에 집중이 가능함.
	     */
	    arr = null;
	    ...
	    //flase -> 각 참조변수는 == 통해서 비교시 주소값을 비교하기 때문에
	    arr4=arr5;
	    System.out.println(arr4==arr5);
