...
public class StringApi {
	...
	public void method01() {
		...
		//String 클래스에 toString메서드는 이미 오버라이딩 되어 있음
		...
		//String 클래스에서는 equals메서드도 이미 오버라이딩 되어있음(주소값 비교가 아니라 문자열 값을 비교하도록)
		
		System.out.println(str1.hashCode());
		System.out.println(str2.hashCode());
		//String 클래스에서 hashCode메서드도 이미 오버라이딩 되어잇음(주소값이 아닌 문자열을 가지고 해시값을 만들도록)
		...
		public void method02() {
			...
			//문자열에 전달괸 문자열이 포함되어 있는지 확인하여 결과를 반환
			System.out.println("포함되어 있니 : "+str1.contains("ello"));
			System.out.println("포함되어 있니 : "+str1.contains("bye"));
			
			//문자열.concat(String str) : String
			//문자열과 전달괸 또다른 문자열을 하나로 합쳐서 새로운 문자열을 리턴
			...
			//문자열에서 대상 문자를 찾아서 변경할 문자로 대체한 후 변환된 문자열을 반환
...
//			System.out.println(stn.nextToken()); 더이상 토큰이 남아있지 않을 때 다음토큰 호출시 예외 발생
			...
			stn  = new StringTokenizer(str,",");
			while(stn.hasMoreElements()) {//남은 토근이 있냐?
				System.out.println(stn.nextToken());
			}
		}
