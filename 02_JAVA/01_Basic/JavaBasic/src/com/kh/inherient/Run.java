...
public class Run {
	 ...
	 * -> 객체 생성시 부모부분이 먼저 생기고 자식 부분 순으로 메모리에 구성되며, 자식객체 내부에 부모객체가 존재하는 것처럼 실행.
	 * 
	 *  장점
	 *  재사용성/생산성 : 코드를 재사용하여 새로운 클래스를 정의할 수 있음
	 *  유지보수성 : 공동코드를 상위에 두어 변경이 용이
	 ...
	 *           - 반환형도 동일
	 * super와 생성자
	 * - 자식 생성자 첫줄에는 super(....)키워드를 통한 부모 생성자 호출 가능
	...
	public static void main(String[] args) {
		...
		/*
		Man m1 = new Man("r김정훈");
		BusinessMan m2 = new BusinessMan("김정훈","KH","강사");
		m2.tellYourInfo();
		m2.tellYourName();
		
//		BusinessMan m2 = new Man(); 자식의 참조변수에 부모객체를 생성할 수는 없다.
///
         */
