...
public class Variable {
	 ...
	 * 변수이름 : 변수를 식별하기 위해 붙혀주는 이름
	 */
	public static void main(String[] args) {
		//정수형 변수 num을 선언
		int num;
		
		//정수형 변수 num에 9라는 값을 대입해라
		num = 9;
		...
		float num2;
		num2 = 3.141221124124f;
		System.out.printf("%.6f",num2);
		
		//----------------------------------------------------
		//원시타입 : 가장 기본적인 데이터타입, 값 자체를 저장햐며 추가적인 속성이 없다.
		 ...
		 */
		  ...
		  * 실수 자료형
		  * float(4) double(8)
		  * 실수를 저장하는 자료형, float보다 double이 더 정밀하게 표현할 수 있음.
		  */
		 float f1 = 4.24f;
		 double dou = 4.24;
		  ...
		  * 문자열
		  * String
		  * 문자열 같은 경우 할당할 메모리크기가 가변적이므로 원시타입이 아닌 객체타입으로 변수를 만들어 사용한다.
		  * 
		  * 원래 객체를 만드는 방식
		  * String 변수명 = new 클래스명(초기값);
		  * 
		  * 문자열은 특별한 객체로 원시타입과 동일하게 생성하고 사용 가능함.
		  * 
		  */
		 ...
		 /*
		  * 이스케이프 시퀀스
		  * 문자열 내에서 탭, 백슬러시, 작은 따음표등을 사용하기 위한 방식
		  */
		 // \t : 탭(tab)
		 // 가로로 일정 간격을 띄움
		  ...
		  * ex)	UserName, TotalPrice
		  * 
		  * 스네이크케이스 : 문자와 문자사이를 언더스코어(_)를 통해서 이어주는 것(파이썬)
		  * ex)user_name, total_price
		  * 
		  * 대문자스네이크케이스 : 스네이크케이스 + 모두 대문자(대부분 언어에서 상수명)
		  * ex) USER_NAME
		  * 
		  * 케밥케이스 : 단어를 소문자로 작성하되 문자와 문자사이를 하이폰(-)으로 이어주는 것(html속성, url, 설정파일등)
		  * ex)user-name
		  */
