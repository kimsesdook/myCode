...
-----------------------------------------------------------------------
/*
뷰 컬럼에 연산식,함수식이 있는 경우 무조건 별칭을 지정해야함.
*/
CREATE OR REPLACE VIEW VW_EMP_JOB
AS (SELECT EMP_ID, EMP_NAME, JOB_NAME,
 DECODE(SUBSTR(EMP_NO, 8, 1), '1','남','2','여')AS "GENDER",
  EXTRACT(YEAR FROM SYSDATE)- EXTRACT(YEAR FROM HIRE_DATE)AS "SERVUCE"
  FROM EMPLOYEE
...
SELECT *
FROM VW_JOB; -- 가상의 논리 테이블(실제 데이터가 저장되어있지 않음)
SELECT * FROM JOB;
...
DML 명령어 사용이 불가한 경우
1)뷰에 없는 컬럼을 조작
2)뷰에 정의되어있지 않은 컬럼 중에 NOT NULL제약조건이 있는 컬럼이 잇는 경우
3)산술/함수식을 사용한 경우
...
AS 서브쿼리
 ...
 >WITH CHECK OPTION : 뷰에 WHERE조건을 강제하여 조건에 맞는 데이터만 삽입,수정 가능하게 함.
