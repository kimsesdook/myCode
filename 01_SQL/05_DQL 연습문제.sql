...
SELECT EMP_NAME, (SUBSTR(EMP_NO, 1 ,7) || REPLACE(SUBSTR(EMP_NO, 8),SUBSTR(EMP_NO, 8),'*******'))AS 주민번호
FROM EMPLOYEE;
--17. EMPLOYEE 테이블에서 사원명, 입사일-오늘, 오늘-입사일 조회 (단, 각 별칭은 근무일수1, 근무일수2가 되도록 하고 모두 정수(내림), 양수가 되도록 처리)
SELECT EMP_NAME, FLOOR(ABS(HIRE_DATE-SYSDATE))AS근무일수,FLOOR(ABS(SYSDATE-HIRE_DATE))AS근무일수2
FROM EMPLOYEE;
...
SELECT *
FROM EMPLOYEE
WHERE FLOOR(SYSDATE - HIRE_DATE)>=20*365;
SELECT *
FROM EMPLOYEE
...
SELECT EMP_NAME,DEPT_CODE, SUBSTR(EMP_NO,1,2)||'년'||SUBSTR(EMP_NO,3,2)||'월'||SUBSTR(EMP_NO,5,2)||'일' AS 생일 ,FLOOR(MONTHS_BETWEEN(SYSDATE,TO_DATE(SUBSTR(EMP_NO,1,6), 'RRMMDD'))/12) AS 나이
FROM EMPLOYEE;
...
SELECT EMP_NAME, SUBSTR(EMP_NO,1,6) AS 앞자리, SUBSTR(EMP_NO,8) AS 뒷자리, SUBSTR(EMP_NO,1,6)+SUBSTR(EMP_NO,8) 
FROM EMPLOYEE
WHERE EMP_ID =201;
