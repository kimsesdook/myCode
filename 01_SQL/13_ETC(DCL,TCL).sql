...
1)사용자생성
CREATE USER 사용자명 INDETFIND BY 비밀번호;;
...
--AUTOCOMMIT여부를 확인
SHOW AUTOCOMMIT;
--오토커밋을 수동설정/ ON->자동으로 DML실행시마다 COMMIT이 진행됨.
SET AUTOCOMMIT OFF;
...
/*
DDL문(CREATE , ALTER, DROP)을 수행하는 순간 기존 트랜잭션은 무조건 커밋됨(실제 DB에 반영)
...
D(DURABILITY)지속성 : 트랜잭션이 COMMIT되면 그 결과는 시스템이 종료되고 영구적으로 보존되어야 한다.
*/
