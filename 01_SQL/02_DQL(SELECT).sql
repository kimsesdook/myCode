...
-컬럼은 테이블에서 저장할떄 속성 = 값으로 저장
=> 테이블은 여러 컬럼으로 구성되고, 각 컬럼은 테이블이 표현하는 데이터의 세부적인 속성을 나타낸다.
<SELECT>
SELECT 컬럼명1, 컬럼명2....
FROM 테이블명
[WHERE 조건]
...
SELECT EMP_NAME, HIRE_DATE, (SYSDATE - HIRE_DATE)*24
FROM EMPLOYEE;
...
--FROM EMPLOYEE;
--위처럼 디스틴드를 중복 사용시 에러가 발생한다.DISTINCT는 한 명령어에서 한번만 사용가능
--DISTINCT는 항상ROW데이터 전체에 대해서 중복을 제거한다.
...
*/
--EMPOYEE 테이블에서 부서코드가 D9인 사원들만 조회(모든컬럼)
SELECT *
FROM EMPLOYEE
WHERE DEPT_CODE = 'D9';
...
조건을 여러개 연결할 때 사용.
[표현법]
조건A AND 조건 B -> 조건 에이와 조건 비가 모두 만족하는 값만 참으로 한다.
조건A OR 조건 B - > 조건 에이와 조건 비중 하나만 만족해도 참으로 한다.
[BETWEEN AND]
조건식에 사용되는 구문
몇이상 몇이하인 범위에 대한 조건을 제시할 때 주로 사용하는 연산자(이상,이하만 ㄱㄴ)
[표현법]
비교대상 컬럼 BETWEEN 하안값 AND상한값;
*/
...
특정패턴을 제시할 때 사용하는 와일드카드라는 특정패턴이 정의되어있다.
...
EX) 비교할 대상 컬럼 LIKE '문자%' : 비교할 대상컬럼 값 중에서 해당 문자로 시작하는 값을 전부 조회
    ...
    비교할 대상 컬럼 LIKE '%문자%' : 비교할 대상컬럼 값 중에서 해당 문자로 포함된 값을 전부 조회
    
2. '_': 1글자를 대체 검색할 때 사용
EX) 비교할 대상 컬럼 LIKE '_문자' : 비교할 대상컬럼 값 문자앞에 아무글자나 딱 한들자가 있는 값을 조회
    비교할 대상 컬럼 LIKE '문자_' : 비교할 대상컬럼 값 문자뒤에 아무글자나 딱 한들자가 있는 값을 조회
    비교할 대상 컬럼 LIKE '_문자_' : 비교할 대상컬럼 값 문자앞뒤에 아무글자나 딱 한들자가 있는 값을 조회
...
FROM EMPLOYEE
--WHERE EMAIL LIKE '____%';->와일드카드로 인식되기 때문에 정상적으로 출력할수 없다
--와일드카드를 직접 문자로 사용할 때는 일반문자로 구분을 해줘야 한다.
...
--8.급여가 200만원 이상이고 500만원 이하인 사원 중에서 입사일이'01/01/01'이상이고 보너스를
-- 받지 않는 사원들의 사번, 사원명, 급여, 입사일, 보너스 조회
