...
SELECT EMP_ID, EMP_NAME, FLOOR((SYSDATE - TO_DATE(SUBSTR(EMP_NO,1,6),'RRMMDD')) /365)AS 나이, DEPT_TITLE, JOB_NAME
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
JOIN JOB USING(JOB_CODE);
...
--JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
--JOIN JOB USING(JOB_CODE)
--WHERE FLOOR((SYSDATE - TO_DATE(SUBSTR(EMP_NO,1,6),'RRMMDD')) /365) = MIN(FLOOR((SYSDATE - TO_DATE(SUBSTR(EMP_NO,1,6),'RRMMDD')) /365);
