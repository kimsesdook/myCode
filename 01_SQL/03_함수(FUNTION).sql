...
>>함수를 사용할 수 있는 위치는 정해짐 : SELECT절 WHERE절 ORDER BY절 GROUP BY절 HAVING절
*/
------------------------------------------단일행 함수-----------------------------------
/*
[문자처리함수]
...
*/
SELECT INSTR('AABAACAACCAA','B')FROM DUAL;
--찾을 위치의 시작값 :1, 순번:1->기본값
SELECT INSTR('AABAACAACCAA','B',1)FROM DUAL;

SELECT INSTR('AABAACAACCAA','B',-1)FROM DUAL;
SELECT INSTR('AABAACAACCAA','B',1,2)FROM DUAL;

SELECT INSTR('AABAACABACBCAA','B',1,3) FROM DUAL;
...
--사원들 중 여자사원들만 이름과 주민등록번호 조회
SELECT EMP_NAME, EMP_NO
FROM EMPLOYEE
WHERE SUBSTR(EMP_NO,8,1) IN('2','4');

--사원들 중 남자사원들만 이름과 주민등록번호 조회
SELECT EMP_NAME, EMP_NO
FROM EMPLOYEE
...
[표현법]
LPAD/RPAD(문자열, 최종적으로 반환할 문자열의 길이, [덧붙이고자 하는 문자])

*/
--30만큼의 길이 중 EMAIL컬럼값은 오른쩍으로 정렬하고 나머지 부분은 공백으로 채운다
...
SELECT LTRIM(' K  H  ')FROM DUAL;--왼쪽에서부터 다른 문자가 나올 때까지 공백을 제거
SELECT LTRIM('123123KH123', '123')FROM DUAL;
SELECT LTRIM('ABCABCKCCBAB', 'ABC')FROM DUAL;---제거하고자하는 문자는 문자열이 아닌 문자들
...
/*
LOWER/UPPER/INITCAP
LOWER :모든 문자열을 소문자로 변경해서 변환
UPPER:모든 문자열을 대문자로 변경 변환
INITCAP : 띄어쓰기 기준 첫글자마다 대문자로 변경한 문자열 반환
*/

SELECT LOWER('Welcome To')FROM DUAL;
SELECT UPPER('Welcome To')FROM DUAL;
SELECT INITCAP('Welcome To')FROM DUAL;
--------------------------------------------
/*
*CONCAT
문자열 두개 전달받아서 하나로 합친후 반환
CONCAT(STR1, STR2)
*/
...
ROUND(값,[위치])
*/
SELECT ROUND(123.456)FROM DUAL;-----기본차수는 소수점 첫번째 자리 반올림
...
ALTER SESSION SET NLS_NANGUAGE = AMERICAN;

ALTER SESSION SET NLS_NANGUAGE = KOREAN;
...
TO_NUMBER(문자,[포맷])
*/
SELECT TO_NUMBER('065658754585458')FROM DUAL;
...
--그 외 직원들은 급여를 5%DLSTKD(SALARY*1.05)
...
/*
CASE문
SQL에서 조건에 따라 값을 분기하고 싶을때 사용하는 구문
...
FROM EMPLOYEE;
--5.COUNT(*|컬럼|DISTINCT컬럼) : 해당 조건에 맞는 행의 개수를 세서 반환
--COUNT(*) : 조회된 결과에 모든 행의 개수를 반환
--COUNT(컬럼) : 제시한 컬럼값이 NULL이 아닌것만 행의 수를 세서 반환
--COUNT(DISTINCT 컬럼) : 해당 컬럼값에서 중복을 제거한 후 행의 개수를 세서 반환
